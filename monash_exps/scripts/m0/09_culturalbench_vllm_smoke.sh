...
output_dir="${M0_EVAL_OUTPUT_DIR:-${runtime_root}/evaluation/culturalbench-easy-smoke-${timestamp}}"
...
exec "${python_bin}" "${repo_root}/monash_exps/src/m0/evaluate_culturalbench_vllm.py" \
    ...
    --runs local_variant_1 \
    --limit "${M0_EVAL_LIMIT:-16}" \
    --batch-size "${M0_EVAL_BATCH_SIZE:-16}" \

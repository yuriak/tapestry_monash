#!/usr/bin/env bash
# Submit this file from the workspace root so Slurm can create the relative
# project log paths below before the batch shell starts.
...
# Slurm runs a copied version of this script from its spool directory, so
# BASH_SOURCE[0] cannot be used to locate the checked-out workspace in a batch
# job.  SLURM_SUBMIT_DIR is the directory in which sbatch was invoked.  Keep a
# BASH_SOURCE fallback so the wrapper can still be exercised directly.
...
if [[ "$#" -lt 1 ]]; then
    echo "Usage: sbatch [cluster options] -J <name> monash_exps/scripts/slurm/submit_job_2gpu.sh <script> [args...]" >&2
    exit 2
fi
...
export SLAKSHNA_EXPECTED_GPUS="${SLAKSHNA_EXPECTED_GPUS:-2}"

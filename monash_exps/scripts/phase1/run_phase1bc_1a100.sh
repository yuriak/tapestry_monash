...
run_id="${1:-$(date '+%Y%m%d_%H%M%S')_${SLURM_JOB_ID:-unknown}_phase1bc_1a100}"
artifact_root="${experiment_root}/artifacts/phase1/${run_id}"
run1_dir="${artifact_root}/phase1b-single-a100-overfit"
run2_dir="${artifact_root}/phase1c-single-a100-resume"
checkpoint_dir="${artifact_root}/checkpoints"
mkdir -p "${run1_dir}" "${run2_dir}"

python "${experiment_root}/src/phase1/prepare_experiment.py" \
    --template "${experiment_root}/configs/phase1/phase1bc_1a100_run1.yaml" \
    --run-dir "${run1_dir}" \
...
python "${experiment_root}/src/phase1/prepare_experiment.py" \
    --template "${experiment_root}/configs/phase1/phase1bc_1a100_run2.yaml" \
    --run-dir "${run2_dir}" \
    --checkpoint-dir "${checkpoint_dir}" \
...
echo "Single-A100 Phase 1B/1C artifact: ${artifact_root}"

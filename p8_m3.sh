...
if [[ "$#" -lt 1 ]]; then
    echo "Usage: bash p8_m3.sh prepare|finish-prepare|reset-run|run <spartan-endpoint-id>|status|audit" >&2
    exit 2
fi
action="$1"
shift
...
site_audit="${cross_root}/site-a-audit.json"
rust_binary="${SLAKSHNA_RUNTIME_ROOT}/cargo-target/slakshna/release/iiitd"
build_manifest="${SLAKSHNA_RUNTIME_ROOT}/manifests/primary/slakshna-build.txt"
...
require_runtime() {
    [[ -x "${rust_binary}" ]] || {
        echo "Missing Slakshna release binary: ${rust_binary}" >&2
        exit 1
    }
    [[ "$(git -C Slakshna rev-parse HEAD)" == "f09eff9a73ae8f1080d4f0b43114b3a8aa5e99bb" ]] || {
        echo "Unexpected Slakshna revision" >&2
        exit 1
    }
    [[ -f "${build_manifest}" ]] || {
        echo "Missing Slakshna build manifest: ${build_manifest}" >&2
        exit 1
    }
    rust_ld_library_path="$(sed -n 's/^runtime_ld_library_path=//p' "${build_manifest}")"
    [[ -n "${rust_ld_library_path}" ]] || {
        echo "Slakshna build manifest has no runtime library path" >&2
        exit 1
    }
}
...
print(f"M3 Phase 8 GPU: {torch.cuda.get_device_name(0)}")
PY
}
...
render_bootstrap_runtime() {
    python "${experiment_root}/src/phase8/prepare_runtime.py" \
        ...
        --site-root "${site_root}" --site site-a \
...
reset_run_state() {
    [[ -d "${site_root}" && -f "${site_root}/global-0/g0-manifest.json" ]] || {
        echo "M3 preparation is incomplete; run prepare first" >&2
        exit 1
    }
    if ss -ltnu 2>/dev/null | grep -Eq ":(${p2p_port}|${ws_port}|${api_port})[[:space:]]"; then
        echo "A Phase 8 M3 port is still active; stop the existing run before reset-run" >&2
        exit 1
    fi
...
show_status() {
    echo "M3 site root       : ${site_root}"
    echo "G0 transfer archive: ${transfer_archive}"
    if [[ -f "${endpoint_file}" ]]; then echo "M3 Iroh EndpointId : $(< "${endpoint_file}")"; fi
    if [[ -f "${node_identity_file}" ]]; then echo "M3 Node Identity    : $(< "${node_identity_file}")"; fi
    if [[ -f "${site_root}/bridge-state.json" ]]; then
        python -m json.tool "${site_root}/bridge-state.json"
    fi
...
case "${action}" in
    prepare)
        [[ "$#" -eq 0 ]] || { echo "prepare takes no arguments" >&2; exit 2; }
        require_runtime
        require_gpu
        [[ ! -e "${site_root}" ]] || {
            echo "Phase 8 M3 site already exists; refusing to overwrite: ${site_root}" >&2
            exit 1
        }
        mkdir -p "${cross_root}"
        limit="$(cpu_limit)"
        echo "=== Prepare private site-a Dolly shard and tokenized inputs ==="
        SLAKSHNA_CPU_LIMIT="${limit}" CUDA_VISIBLE_DEVICES=0 python \
            "${experiment_root}/src/phase8/prepare_site.py" \
            --site site-a --site-root "${site_root}" \
            2>&1 | tee "${cross_root}/site-a-prepare.log"
        echo "=== Create and install the common G0 bundle ==="
        CUDA_VISIBLE_DEVICES=0 python "${experiment_root}/src/phase8/g0_bundle.py" create \
            --site-root "${site_root}" --output-dir "${transfer_dir}"
        python "${experiment_root}/src/phase8/g0_bundle.py" install \
            --bundle-dir "${transfer_dir}" --site-root "${site_root}"
        tar -C "${transfer_dir}" -czf "${transfer_archive}" \
            global_adapter.safetensors global_adapter.pth g0-manifest.json creation-audit.json
        sha256sum "${transfer_archive}" | tee "${transfer_archive}.sha256"
        echo "=== Generate and persist the M3 Iroh identity ==="
        render_bootstrap_runtime
        bootstrap_identity
        echo "M3 PHASE8 PREPARE PASSED"
        show_status
        ;;
    finish-prepare)
        [[ "$#" -eq 0 ]] || { echo "finish-prepare takes no arguments" >&2; exit 2; }
        require_runtime
        require_gpu
        for required in \
            "${site_root}/site-manifest.json" \
            "${site_root}/global-0/g0-manifest.json" \
            "${transfer_dir}/g0-manifest.json" \
            "${transfer_archive}" \
            "${transfer_archive}.sha256" \
            "${runtime_dir}/node.toml"; do
            [[ -f "${required}" ]] || { echo "Missing partial-prepare artifact: ${required}" >&2; exit 1; }
        done
        [[ -d "${data_dir}" ]] || { echo "Missing identity data directory: ${data_dir}" >&2; exit 1; }
        sha256sum --check "${transfer_archive}.sha256"
        python "${experiment_root}/src/phase8/g0_bundle.py" inspect \
            --bundle-dir "${transfer_dir}" --site-root "${site_root}" >/dev/null
        if [[ -f "${endpoint_file}" && -f "${node_identity_file}" ]]; then
            echo "M3 identity preparation was already completed."
        else
            echo "=== Resume only the M3 identity bootstrap ==="
            bootstrap_identity
        fi
        echo "M3 PHASE8 PREPARE PASSED"
        show_status
    ...
    reset-run)
        ...
        require_runtime
        require_gpu
        echo "=== Reset failed-run state and issue a fresh M3 identity ==="
        reset_run_state
        echo "M3 PHASE8 RESET PASSED"
        show_status
        ;;
    run)
        [[ "$#" -eq 1 ]] || { echo "Usage: bash p8_m3.sh run <spartan-endpoint-id>" >&2; exit 2; }
        peer_endpoint="$1"
        [[ "${peer_endpoint}" =~ ^[0-9a-f]{64}$ ]] || { echo "Invalid Spartan EndpointId" >&2; exit 2; }
        require_runtime
        require_gpu
        [[ -f "${endpoint_file}" && -f "${site_root}/global-0/g0-manifest.json" ]] || {
            echo "Run M3 prepare first" >&2; exit 1;
        }
        [[ ! -e "${site_root}/bridge-state.json" ]] || {
            echo "Bridge state already exists; refusing an implicit resume" >&2; exit 1;
        }
        render_final_runtime "${peer_endpoint}"
        remaining=$((epoch_duration - ($(date +%s) % epoch_duration)))
        if (( remaining < 120 )); then
            echo "Waiting $((remaining + 1))s to avoid starting inside a partial federation interval"
            sleep $((remaining + 1))
        fi
        node_pid=""
        cleanup() {
        ...
        echo "=== Start Playit ingress and M3 Slakshna node ==="
        bash "${experiment_root}/scripts/phase8/playit_agent.sh" start
        start_node "${rust_log}"
        if ! wait_api "${node_pid}" "${site_root}/api-status.initial.json"; then
            tail -n 180 "${rust_log}" >&2
            exit 1
        fi
        actual_endpoint="$(json_value "${site_root}/api-status.initial.json" endpoint_id)"
        [[ "${actual_endpoint}" == "$(< "${endpoint_file}")" ]] || {
            echo "M3 Iroh identity changed after bootstrap" >&2; exit 1;
        }
        echo "M3 PHASE8 READY"
        echo "EndpointId : ${actual_endpoint}"
        echo "Public seed: ${actual_endpoint}@${public_seed_address}"
        echo "Start the Spartan run command now."
        echo "=== Wait for the authenticated Spartan mesh peer ==="
        mesh_ready=0
        for _ in $(seq 1 600); do
            kill -0 "${node_pid}" 2>/dev/null || break
            curl --silent --fail --connect-timeout 2 \
                "http://127.0.0.1:${api_port}/peers" > "${site_root}/api-peers.tmp.json" || true
            if python - "${site_root}/api-peers.tmp.json" "${peer_endpoint}" <<'PY' 2>/dev/null
import json, sys
value = json.load(open(sys.argv[1], encoding="utf-8"))
...
PY
        ...
        for poll in $(seq 1 600); do
            ...
            if (( poll % 6 == 0 )); then
                echo "M3 Phase 8 progress: elapsed=$((poll * 10))s rounds=${rounds}/5 finalized=${finalized}"
            fi
            if [[ "${rounds}" == 5 && "${finalized}" == true ]]; then completed=1; break; fi
            sleep 10
        done
        [[ "${completed}" == 1 ]] || { echo "M3 Phase 8 timed out" >&2; exit 1; }
        sleep 5
        ...
        stop_pid "${node_pid}"; node_pid=""
        bash "${experiment_root}/scripts/phase8/playit_agent.sh" stop
        trap - EXIT INT TERM
        python "${experiment_root}/src/phase8/audit_site.py" \
            --site-root "${site_root}" --output "${site_audit}"
        printf '%s\n' "${cross_root}" > "${experiment_root}/artifacts/phase8/latest-cross-cluster-m3.txt"
        echo "M3 PHASE8 SITE RUN PASSED"
